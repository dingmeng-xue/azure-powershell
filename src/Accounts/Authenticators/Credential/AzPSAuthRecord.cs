...
namespace Microsoft.Azure.PowerShell.Authenticators.Credential
{
    ...
    public class AzPSAuthRecord
    {
        ...
        public AzPSAuthRecord()
        {
        }

        public AzPSAuthRecord(AuthenticationResult authResult, string clientId)
        {
        ...
        public AzPSAuthRecord(string username, string authority, string homeAccountId, string tenantId, string clientId)
        {
            Username = username;
            Authority = authority;
        ...
        /// <summary>
        /// Serializes the <see cref="AzPSAuthRecord"/> to the specified <see cref="Stream"/>.
        /// </summary>
        /// <param name="stream">The <see cref="Stream"/> which the serialized <see cref="AzPSAuthRecord"/> will be written to.</param>
        ...
        /// Serializes the <see cref="AzPSAuthRecord"/> to the specified <see cref="Stream"/>.
        /// </summary>
        /// <param name="stream">The <see cref="Stream"/> to which the serialized <see cref="AzPSAuthRecord"/> will be written.</param>
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> controlling the request lifetime.</param>
        ...
        /// <summary>
        /// Deserializes the <see cref="AzPSAuthRecord"/> from the specified <see cref="Stream"/>.
        /// </summary>
        /// <param name="stream">The <see cref="Stream"/> from which the serialized <see cref="AzPSAuthRecord"/> will be read.</param>
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> controlling the request lifetime.</param>
        public static AzPSAuthRecord Deserialize(Stream stream, CancellationToken cancellationToken = default)
        {
            if (stream is null)
                throw new ArgumentNullException(nameof(stream));
        ...
        /// <param name="stream">The <see cref="Stream"/> from which the serialized <see cref="AzPSAuthRecord"/> will be read.</param>
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> controlling the request lifetime.</param>
        public static async Task<AzPSAuthRecord> DeserializeAsync(Stream stream, CancellationToken cancellationToken = default)
        {
            if (stream is null)
                throw new ArgumentNullException(nameof(stream));
        ...
        private static async Task<AzPSAuthRecord> DeserializeAsync(Stream stream, bool async, CancellationToken cancellationToken)
        {
            var authProfile = new AzPSAuthRecord();
                ...
                foreach (JsonProperty prop in doc.RootElement.EnumerateObject())
                {
                    switch (prop.Name)
                    {
                        ...
                        case VersionPropertyName:
                            ...
                            {
                                throw new InvalidOperationException($"Attempted to deserialize an {nameof(AzPSAuthRecord)} with a version that is not the current version. Expected: '{CurrentVersion}', Actual: '{authProfile.Version}'");
                            }
                            break;
                    }
                }
