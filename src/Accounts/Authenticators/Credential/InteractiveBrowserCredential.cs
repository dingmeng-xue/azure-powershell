...
namespace Microsoft.Azure.PowerShell.Authenticators.Credential
{
    ...
    public class InteractiveBrowserCredential : TokenCredential
    {
        ...
        internal AzPSAuthRecord Record { get; private set; }
            ...
            : this(options?.TenantId, options?.ClientId ?? Constants.DeveloperSignOnClientId, options, null)
        {
            ...
            {
                var record = options.AuthenticationRecord;
                Record = new AzPSAuthRecord(record.Username, record.Authority, record.HomeAccountId, record.TenantId, record.ClientId);
            }
        }
        ...
        /// <returns>The result of the authentication request, containing the acquired <see cref="AccessToken"/>, and the <see cref="AzPSAuthRecord"/> which can be used to silently authenticate the account.</returns>
        public virtual AzPSAuthRecord Authenticate(CancellationToken cancellationToken = default)
        {
        ...
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> controlling the request lifetime.</param>
        /// <returns>The result of the authentication request, containing the acquired <see cref="AccessToken"/>, and the <see cref="AzPSAuthRecord"/> which can be used to silently authenticate the account.</returns>
        public virtual async Task<AzPSAuthRecord> AuthenticateAsync(CancellationToken cancellationToken = default)
        {
            // get the default scope for the authority, throw if no default scope exists
            string defaultScope = AzureAuthorityHosts.GetDefaultScope(Pipeline.AuthorityHost) ?? throw new CredentialUnavailableException(NoDefaultScopeMessage);
        ...
        /// <returns>The <see cref="AzPSAuthRecord"/> of the authenticated account.</returns>
        public virtual AzPSAuthRecord Authenticate(TokenRequestContext requestContext, CancellationToken cancellationToken = default)
        {
        ...
        /// <param name="requestContext">The details of the authentication request.</param>
        /// <returns>The <see cref="AzPSAuthRecord"/> of the authenticated account.</returns>
        public virtual async Task<AzPSAuthRecord> AuthenticateAsync(TokenRequestContext requestContext, CancellationToken cancellationToken = default)
        {
        ...
        private async Task<AzPSAuthRecord> AuthenticateImplAsync(bool async, TokenRequestContext requestContext, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<AccessToken> GetTokenViaBrowserLoginAsync(TokenRequestContext context, bool async, CancellationToken cancellationToken)
        {
            ...
            Record = new AzPSAuthRecord(result, ClientId);
