...
namespace Microsoft.Azure.PowerShell.Authenticators.Credential
{
    internal class MsalPublicClient : MsalClientBase<IPublicClientApplication>
    {
        ...
        public async ValueTask<AuthenticationResult> AcquireTokenSilentAsync(string[] scopes, string claims, AzPSAuthRecord record, bool async, CancellationToken cancellationToken)
        {
            return await AcquireTokenSilentCoreAsync(scopes, claims, record, async, cancellationToken).ConfigureAwait(false);
        }

        protected virtual async ValueTask<AuthenticationResult> AcquireTokenSilentCoreAsync(string[] scopes, string claims, AzPSAuthRecord record, bool async, CancellationToken cancellationToken)
        {
            IPublicClientApplication client = await GetClientAsync(async, cancellationToken).ConfigureAwait(false);
